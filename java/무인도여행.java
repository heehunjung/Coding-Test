...
class Solution {
    ...
    private int bfs(int x,int y) {
        ...
        while(!q.isEmpty()) {
            int[] current = q.poll();
            result += Integer.parseInt(myMaps[current[0]][current[1]]);
    ...
    private boolean isValidXY(int x,int y) {
        if (0<= x && x < myMaps.length && 0 <= y && y < myMaps[0].length) {
            if(!visited[x][y] && !myMaps[x][y].equals("X")){
                return true;
            }
        }
        return false;
